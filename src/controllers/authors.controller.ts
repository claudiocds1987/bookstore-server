...
export const getAuthorByName = async (
  ...
  if (!req.params.name) {
    return res.status(400).send({
      message:
        "FALTA CONTENIDO EN EL CUERPO, falta el nombre para buscar el autor por nombre",
    });
  }

  try {
    const response: QueryResult = await pool.query(
      `SELECT * FROM authors WHERE name ILIKE '%${req.params.name}%'`
    );
...
export const filterAuthorsByName = async (
  ...
  if (!req.params.name) {
    return res.status(400).send({
      message:
        "FALTA CONTENIDO EN EL CUERPO, falta el nombre de autor para buscar el autor",
    });
  }

  try {
    const response: QueryResult = await pool.query(
      `SELECT * from authors WHERE authors.name iLIKE '%${req.params.name}%'`
    );
    return res.json(response.rows);
  } catch (e) {
    console.log(e);
...
export const existAuthorByName = async (
  req: Request,
  res: Response
): Promise<Response> => {
  console.log(
    "Nombre de el autor para evaluar si existe autor: " + req.params.name
  );
