...
export const createOrderDetail = async (
  ...
  const response: QueryResult = await pool.query(
    "INSERT INTO orders_detail (id_order, id_product, product_price, product_quantity) VALUES ($1, $2, $3, $4)",
    [idOrder, idProduct, price, cantidad]
  );
  return res.json({
...
export const getOrderDetail = async (
  ...
  try {
    ...
    const response: QueryResult = await pool.query(
      "SELECT * FROM orders_detail WHERE orders_detail.id_order = $1",
      [id]
    );
